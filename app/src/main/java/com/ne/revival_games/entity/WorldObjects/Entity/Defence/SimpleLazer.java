...
public class SimpleLazer extends Lazer {
    ...
    public SimpleLazer(Vector2 start, Vector2 end, double width, double health,
                       double speed, MyWorld world, boolean addToWorld) {
        super(width, 0, speed, health, world, addToWorld);
    ...
    public SimpleLazer(Vector2 start, double angle, double length, double width, double health,
                       double speed, MyWorld world, boolean addToWorld) {
        super(width, 0, speed, health, world, addToWorld);
        start = new Vector2(start.x/ MyWorld.SCALE, start.y/MyWorld.SCALE);
    ...
    public Projectile returnCustomizedCopy(Projectile project, Vector2 location, double direction, double speed, MyWorld world) {
        if(project.getClass() == this.getClass()){
            ...
            return new SimpleLazer(location, Math.toRadians(direction),
                    lazer.lazer_length*MyWorld.SCALE, lazer.lazer_width*MyWorld.SCALE, lazer.health, lazer.lazer_speed, world, true);
        }
        return null;
    }
